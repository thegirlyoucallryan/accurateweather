...
export const InformationPanel: React.FC = () => {
  ...
  if (!data || data.forecast.length === 0)
    return (
      <div className="text-center mt-10">
        <div className="text-purple-800 text-5xl">
          Very Accurate Weather App
        </div>
        <p className="mt-8 text-xl text-gray-500">
          Search above by zip code. Feel free to change your unit preferences
          below.
        </p>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-xl p-4 w-full h-full flex flex-col justify-center place-self-center mt-4">
      {location && (
        <div className="uppercase font-bold sm:text-sm md:text-base">
          {location.city},{" "}
          {stateAbbreviations[location.region] || location.region}
        </div>
      )}
        ...
        <div className="w-full flex justify-between items-center mt-2">
          <div className="flex items-center gap-4">
            <div className="text-xl flex flex-row">
              ...
              <div className="text-purple-900 lg:text-5xl sm:text-2xl font-bold mt-2">
                {unit}
              </div>
            </div>
          </div>
          {current.icon &&
            React.createElement(iconMap[current.icon] || iconMap[1000], {
              className: "w-40 h-40 text-purple-900",
            })}
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-5 md:grid-cols-2 sm:grid-cols-1 gap-3 mt-6">
        {forecast.map((f) => (
          ...
          >
            {f.icon &&
              React.createElement(iconMap[f.icon] || iconMap[1000], {
                className: "w-18 h-18 text-purple-900",
              })}
            <div className="lg:text-3xl md:text-2xl sm:text-xl font-bold mt-1 flex flex-row items-center">
              {unit === "F" ? f.maxTempF : f.maxTempC}°{" "}
              <p className="text-xs ml-1">{unit}</p>
            </div>
